...
export async function resolveOptions(options, viteConfig) {
    ...
    mode = (process['env']['NODE_ENV'] || 'production'), srcDir = 'public', outDir = viteConfig.build.outDir || 'dist', injectRegister = 'auto', registerType = 'prompt', filename = 'sw.js', manifestFilename = 'manifest.webmanifest', strategies = 'generateSW', minify = false, base = viteConfig.base, includeAssets = undefined, includeManifestIcons = true, useCredentials = false, disable = false, devOptions = { enabled: false, type: 'classic' }, selfDestroying = false, integration = {}, } = options;
